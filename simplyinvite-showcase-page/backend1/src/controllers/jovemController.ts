...
// Criar novo projeto
export const criarProjeto: RequestHandler = async (req, res) => {
  const { titulo, descricao, tecnologias, linkRepositorio, linkDeploy } = req.body;
...
// Atualizar projeto
export const atualizarProjeto: RequestHandler = async (req, res) => {
  const { id } = req.params;
  const { titulo, descricao, tecnologias, linkRepositorio, linkDeploy } = req.body;
  ...
  try {
    ...
    if (!projeto) {
      res.status(404).json({ message: 'Projeto não encontrado ou não pode ser editado' });
      return;
    }
...
// Excluir projeto
export const excluirProjeto: RequestHandler = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (!projeto) {
      res.status(404).json({ message: 'Projeto não encontrado ou não pode ser excluído' });
      return;
    }
...
export const buscarMeusProjetos: RequestHandler = async (req, res) => {
  try {
...
// Buscar feedbacks
export const buscarFeedbacks: RequestHandler = async (req, res) => {
  try {
    const usuarioId = (req as any).usuario.id;
...
// Buscar convites
export const buscarConvites: RequestHandler = async (req, res) => {
  try {
    const jovemId = (req as any).usuario.id;
...
// Aceitar convite
export const aceitarConvite: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
    const jovemId = (req as any).usuario.id;
    ...
    if (!convite) {
      return res.status(404).json({ message: 'Convite não encontrado ou já respondido' });
    }

    await convite.update({ status: 'aceito' });

    // Atualizar o empresaId do usuário
...
// Recusar convite
export const recusarConvite: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
    const jovemId = (req as any).usuario.id;
    const convite = await db.Convite.findOne({
...
// Buscar evolução
export const buscarEvolucao: RequestHandler = async (req, res) => {
  try {
    const usuarioId = (req as any).usuario.id;
...
export const completarOnboarding: RequestHandler = async (req, res) => {
  try {
    const usuarioId = (req as any).usuario.id;
    const {
      nomeCompleto,
